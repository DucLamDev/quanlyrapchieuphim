...
import { 
  ...
  XCircle, Search, Clock, User, MapPin, Phone, Mail 
} from 'lucide-react'
...
export default function StaffDashboard() {
  ...
  const fetchRecentBookings = async () => {
    try {
      ...
      // Filter out checked-in bookings
      const uncheckedBookings = (response.bookings || []).filter(
        (booking: any) => !booking.isCheckedIn
      )
      setBookings(uncheckedBookings)
    } catch (error) {
  ...
  const fetchTodayStats = async () => {
    try {
      ...
      setAllBookingsToday(todayBookings)
      
      // Calculate statistics
      const totalRevenue = todayBookings.reduce((sum: number, b: any) => sum + (b.totalAmount || 0), 0)
      const totalTickets = todayBookings.reduce((sum: number, b: any) => sum + (b.seats?.length || 0), 0)
      const pendingCheckIns = todayBookings.filter((b: any) => 
        (b.status === 'confirmed' || b.status === 'paid') && b.isCheckedIn !== true
      ).length
  ...
  const handleCheckIn = async () => {
    ...
    try {
      await api.checkInBooking(selectedBooking._id)
      toast({
      ...
      toast({
        title: 'Lỗi',
        description: error.response?.data?.message || 'Không thể check-in',
        variant: 'destructive'
      })
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {(user as any)?.cinemaId && (
          <div className="mb-8 bg-gradient-to-r from-cinema-600/20 to-purple-600/20 border border-cinema-500/30 rounded-xl p-6">
            <div className="flex items-start gap-4">
              <div className="w-14 h-14 bg-cinema-600 rounded-full flex items-center justify-center flex-shrink-0">
                <MapPin className="w-7 h-7 text-white" />
              </div>
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  <p className="text-sm text-gray-400">Bạn đang làm việc tại</p>
                </div>
                <h2 className="text-2xl font-bold text-cinema-400 mb-2">{(user as any).cinemaId.name}</h2>
                <div className="flex flex-wrap gap-4 text-sm">
                  <div className="flex items-center gap-2 text-gray-300">
                    <MapPin className="w-4 h-4 text-gray-500" />
                    <span>
                      {typeof (user as any).cinemaId.location === 'string' 
                        ? (user as any).cinemaId.location 
                        : (user as any).cinemaId.location?.address || 'N/A'}
                    </span>
                  </div>
                  {(user as any).cinemaId.phone && (
                    <div className="flex items-center gap-2 text-gray-300">
                      <Phone className="w-4 h-4 text-gray-500" />
                      <span>{(user as any).cinemaId.phone}</span>
                    </div>
                  )}
                  {(user as any).cinemaId.email && (
                    <div className="flex items-center gap-2 text-gray-300">
                      <Mail className="w-4 h-4 text-gray-500" />
                      <span>{(user as any).cinemaId.email}</span>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 border border-blue-500/20 rounded-lg p-6">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm text-gray-400">Vé hôm nay</p>
              <Ticket className="w-5 h-5 text-blue-400" />
            </div>
            <p className="text-3xl font-bold text-blue-400">{stats.totalBookings}</p>
            <p className="text-xs text-gray-500 mt-1">Tổng số vé đã đặt</p>
          </div>
          ...
          <div className="bg-gradient-to-br from-green-500/10 to-green-600/5 border border-green-500/20 rounded-lg p-6">
            ...
            <p className="text-xs text-gray-500 mt-1">Tổng doanh thu hôm nay</p>
          </div>
          ...
          <div className="bg-gradient-to-br from-purple-500/10 to-purple-600/5 border border-purple-500/20 rounded-lg p-6">
            ...
            <p className="text-3xl font-bold text-purple-400">{stats.totalTickets}</p>
            <p className="text-xs text-gray-500 mt-1">Số lượng ghế đã bán</p>
          </div>

          <div className="bg-gradient-to-br from-orange-500/10 to-orange-600/5 border border-orange-500/20 rounded-lg p-6">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm text-gray-400">Chờ check-in</p>
              <Clock className="w-5 h-5 text-orange-400" />
            </div>
            <p className="text-3xl font-bold text-orange-400">{stats.pendingCheckIns}</p>
            <p className="text-xs text-gray-500 mt-1">Vé chưa check-in</p>
          </div>
        </div>
