...
export default function BookingPage() {
  ...
  const fetchShowtimeDetails = async () => {
    try {
      ...
      let errorMessage = 'Không thể tải thông tin suất chiếu'
      
      if (error.response?.status === 400) {
        errorMessage = 'Mã suất chiếu không hợp lệ hoặc dữ liệu bị lỗi'
  ...
  const handleSeatSelect = (seat: any) => {
    ...
    if (isSelected) {
      ...
      // Use price based on seat type from showtime
      const seatPrice = showtime?.price?.[seat.type] || showtime?.price?.standard || 100000
      addSeat({ ...seat, price: seatPrice })
    }
  }
  ...
  const handleApplyPromo = async () => {
    ...
    try {
      ...
      const response = await api.validatePromotionCode({
        code: promoCode,
      ...
      toast({
        title: 'Thành công',
        description: `Đã áp dụng mã giảm ${response.promotion.discountType === 'percentage' ? response.promotion.discountValue + '%' : formatCurrency(response.promotion.discountValue)}`
      })
    } catch (error: any) {
      toast({
        title: 'Lỗi',
        description: error.response?.data?.message || 'Mã khuyến mãi không hợp lệ',
        variant: 'destructive'
      })
    } finally {
  ...
  const calculateDiscount = () => {
    ...
    if (appliedPromo.discountType === 'percentage') {
      return Math.min(total * (appliedPromo.discountValue / 100), appliedPromo.maxDiscount || total)
    }
                ...
                {!appliedPromo ? (
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 bg-gray-800 rounded text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cinema-500"
                    />
                    <Button
                  ...
                  <div className="flex justify-between items-center p-3 bg-green-500/10 rounded border border-green-500/20">
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{appliedPromo.name}</p>
                    </div>
                    <Button
