...
export default function EditShowtimePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [moviesRes, cinemasRes, showtimeRes] = await Promise.all([
        api.getMovies(),
        api.getCinemas(),
        api.getShowtime(params.id as string)
      ])
      
      setMovies(moviesRes.movies || [])
      setCinemas(cinemasRes.cinemas || [])
      
      const showtime = showtimeRes.showtime
      if (showtime) {
        ...
        setFormData({
          ...
          priceStandard: showtime.price?.standard?.toString() || '80000',
          priceVIP: showtime.price?.vip?.toString() || '120000',
          priceCouple: showtime.price?.couple?.toString() || '150000'
        })
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      await api.updateShowtime(params.id as string, showtimeData)
      
      toast({
        title: 'Thành công',
        description: 'Đã cập nhật suất chiếu'
      })
      ...
      toast({
        title: 'Lỗi',
        description: error.response?.data?.message || 'Không thể cập nhật suất chiếu',
        variant: 'destructive'
      })
    } finally {
      setSaving(false)
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black text-white">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold">Chỉnh sửa suất chiếu</h1>
          <p className="text-gray-400 mt-2">Cập nhật thông tin suất chiếu</p>
        </div>
