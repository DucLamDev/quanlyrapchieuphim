...
export function SeatSelector({ showtime, selectedSeats, onSeatSelect }: SeatSelectorProps) {
  ...
  useEffect(() => {
    if (showtime?.room?.seats) {
      ...
      // Generate default seat layout with real booked seats
      generateDefaultSeats()
    }
  }, [showtime, showtime?.bookedSeats])
  ...
  const generateDefaultSeats = () => {
    ...
    // Get real booked seats from showtime API
    const bookedSeatsFromAPI = new Set(
      (showtime?.bookedSeats || []).map((s: any) => `${s.row}${s.number}`)
    )

    // Get prices from showtime, fallback to defaults
    const priceStandard = showtime?.price?.standard || 100000
    const priceVip = showtime?.price?.vip || 150000
    const priceCouple = showtime?.price?.couple || 200000

    for (let i = 0; i < rows; i++) {
      const row: Seat[] = []
      for (let j = 1; j <= cols; j++) {
        // VIP seats (rows B-C), Couple seats (rows I-J), Standard (D-H)
        const type = i >= 1 && i <= 2 ? 'vip' : i >= 8 ? 'couple' : 'standard'
        const price = type === 'vip' ? priceVip : type === 'couple' ? priceCouple : priceStandard
        const seatCode = `${rowLabels[i]}${j}`
        
        row.push({
          row: rowLabels[i],
          number: j,
          type,
          status: bookedSeatsFromAPI.has(seatCode) ? 'booked' : 'available',
          price
        })
      }
      layout.push(row)
