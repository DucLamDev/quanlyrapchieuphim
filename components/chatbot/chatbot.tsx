  ...
  const handleSendMessage = async (content?: string) => {
    ...
    try {
      ...
      const botMessage: Message = {
        ...
        movieCards: response.movieCards,
                ...
                >
                  <div className={`flex gap-2 max-w-[80%] ${message.type === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    <div>
                      ...
                      {/* Movie Cards */}
                      {message.movieCards && message.movieCards.length > 0 && (
                        <div className="mt-3 space-y-3">
                          ...
                          {message.movieCards.slice(0, 3).map((movie) => (
                            <div
                              key={movie._id}
                              className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden hover:border-cinema-500 transition-colors"
                            >
                              <div className="flex gap-3 p-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <button
                                    ...
                                    className="w-full px-3 py-1.5 bg-gradient-to-r from-cinema-600 to-purple-600 hover:from-cinema-700 hover:to-purple-700 text-white text-xs font-medium rounded transition-all"
                                  >
                                    🎟️ Đặt vé ngay
                                  </button>
                                </div>
                          ...
                          {message.movieCards.length > 3 && (
                            <button
                              onClick={() => window.location.href = '/movies'}
                              className="w-full text-center text-xs text-cinema-400 hover:text-cinema-300 py-2"
                            >
                              Xem thêm {message.movieCards.length - 3} phim khác →
                            </button>
                          )}
                        </div>
                      )}
