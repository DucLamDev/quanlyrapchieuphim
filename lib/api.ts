...
class APIClient {
  ...
  async checkInBooking(id: string) {
    const response = await this.client.put(`/bookings/${id}/checkin`)
    return response.data
  }
  ...
  async createCinema(data: any) {
    const response = await this.client.post('/cinemas', data)
    return response.data
  }

  async updateCinema(id: string, data: any) {
    const response = await this.client.put(`/cinemas/${id}`, data)
    return response.data
  }

  async deleteCinema(id: string) {
    const response = await this.client.delete(`/cinemas/${id}`)
    return response.data
  }
  ...
  // VNPay Payment
  async createVNPayPayment(data: any) {
    const response = await this.client.post('/payments/vnpay/create', data)
    return response.data
  }

  async verifyVNPayCallback(queryParams: any) {
    const response = await this.client.get('/payments/vnpay/callback', { params: queryParams })
    return response.data
  }

  async checkVNPayStatus(bookingId: string) {
    const response = await this.client.get(`/payments/vnpay/status/${bookingId}`)
    return response.data
  }
  ...
  async exportReport(params: any) {
    ...
    if (format === 'pdf') {
      ...
      const token = localStorage.getItem('token')
      const url = `${this.client.defaults.baseURL}/revenue/export?${queryString}`
      
      // Open in new window with auth token
      const newWindow = window.open('', '_blank')
      if (newWindow) {
        newWindow.location.href = url
        // Set auth header via fetch to ensure proper authentication
        fetch(url, {
          headers: {
      ...
      return { success: true, message: 'Opening PDF report...' }
    }
  ...
  // Admin Showtime Generation
  async generateShowtimesToday() {
    const response = await this.client.post('/admin/showtimes/generate-today')
    return response.data
  }

  async generateShowtimesNextDay() {
    const response = await this.client.post('/admin/showtimes/generate')
    return response.data
  }

  async generateShowtimesMultipleDays(days: number = 7) {
    const response = await this.client.post('/admin/showtimes/generate-multiple', { days })
    return response.data
  }

  async ensureShowtimesExist() {
    const response = await this.client.post('/admin/showtimes/ensure')
    return response.data
  }

  async getShowtimeStats() {
    const response = await this.client.get('/admin/showtimes/stats')
    return response.data
  }

  async cleanupOldShowtimes() {
    const response = await this.client.post('/admin/showtimes/cleanup')
    return response.data
  }

  async regenerateShowtimesToday() {
    const response = await this.client.post('/admin/showtimes/regenerate-today')
    return response.data
  }
